...
static void PCHallOfFame_DrawAllPokemon(PCHallOfFameApp *pcHallOfFameApp)
{
    ...
    for (i = 0; i < pcHallOfFameScreen->pokemonCount; i++) {
        Pokemon_InitWith(pcHallOfFameApp->mon, pcHallOfFameScreen->pokemon[i].species, pcHallOfFameScreen->pokemon[i].level, INIT_IVS_RANDOM, 0, 0, TRUE, pcHallOfFameScreen->pokemon[i].personality, 0, OTID_SET, pcHallOfFameScreen->pokemon[i].OTID);
